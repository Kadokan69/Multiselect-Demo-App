...
// Очистка выбранных опций
  ...
  return (
    <div className="w-100 relative" ref={ref}>
      <div className="flex bg-[#4c4c4c] rounded-[6px] items-center">
        <ul
      ...
      {isOpen && (
        <div className="bg-[#4c4c4c] rounded-[6px] absolute w-full">
          <div className="p-1.5">
            <input
              type="text"
              placeholder='Search'
          ...
          <ul className="multiselect-scrollbar max-h-[100px] overflow-auto text-left p-2.5 pt-0 ">
            {filterOptions.length? filterOptions.map((item) => (
              <li
                key={item.region}
